...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'business_allowed_ip_id' => ! empty($input['business_allowed_ip_id']) ? $input['business_allowed_ip_id'] : null,
            'type' => $sale_type,
            'status' => $input['status'],
            ...
            'is_kitchen' => ! empty($input['is_kitchen']) ? 1 : 0,
                    ...
                    if($payment['method']  == 'card'){
                        ...
                        */
                        ...
                        if($transaction->is_split_payment == 1){
                            ...
                            $card_charge_amount = ($payment_data['amount'] * $card_charge / 100);
                            $card_charge_amount = sprintf('%0.2f', $card_charge_amount);
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if (! empty($transaction->tax)) {
            if ($transaction->tax->is_tax_group) {
                $order_taxes = $this->sumGroupTaxDetails($this->groupTaxDetails($transaction->tax, $transaction->tax_amount));
            } else {
                $order_taxes[$transaction->tax->name] = $transaction->tax_amount;
            }
        }
        $output['order_taxes'] = $order_taxes;
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $line_array = [
                ...
                'order_receipt_item_printed_flag' => $line->order_receipt_item_printed_flag,
                ...
                'is_kitchen' => isset($product->product_custom_field1) ? $product->product_custom_field1 : 0,
                'allow_for_print_order_receipt' => isset($product->allow_for_print_order_receipt) ? $product->allow_for_print_order_receipt : 0,
    ...
    public function getInvoiceNumber($business_id, $status, $location_id, $invoice_scheme_id = null, $sale_type = null)
    {
        if ($status == 'final') {
            ...
            //Count
            $counter = $scheme->invoice_count;

            if($scheme->is_enable) {
                $today = date('Y-m-d');
                $updated_at = date_format($scheme->updated_at, 'Y-m-d');

                $date_last_updated = strtotime($updated_at);
            ...
            $count = $scheme->start_number + $counter;
    ...
    public function mapPurchaseSellForStockAdjustment($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        if (empty($transaction_lines)) {
            return false;
        }
        ...
        foreach ($transaction_lines as $line) {
            ...
            $allow_stock_adjustment_on_out_of_stock_items = isset($common_settings['allow_stock_adjustment_on_out_of_stock_items']) ? $common_settings['allow_stock_adjustment_on_out_of_stock_items'] : 0;
            //$allow_stock_adjustment_on_out_of_stock_items = 1; 
            if (!$allow_stock_adjustment_on_out_of_stock_items) {
                //If overselling not allowed through exception else create mapping with blank purchase_line_id
                if (! $allow_overselling) {
                    $variation = Variation::find($line->variation_id);
                    $mismatch_name = $product->name;
